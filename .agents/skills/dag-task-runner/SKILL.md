...
This skill can run from a project skill (`.cursor/skills/dag-task-runner` or `.agents/skills/dag-task-runner`), a Claude Code skill (`.claude/skills/dag-task-runner` or `~/.claude/skills/dag-task-runner`), or a personal Cursor skill (`~/.cursor/skills/dag-task-runner`). The installed runner entry point is `scripts/run_dag.ts` inside the skill directory. Set `DAG_RUNNER_DIR` to override the auto-detected `scripts` directory.
...
resolve_runner_dir() {
  ...
  for dir in \
    ...
    "$PWD/.agents/skills/dag-task-runner/scripts" \
    "${git_root:+$git_root/.agents/skills/dag-task-runner/scripts}" \
    "$PWD/.claude/skills/dag-task-runner/scripts" \
    "${git_root:+$git_root/.claude/skills/dag-task-runner/scripts}" \
    "$HOME/.cursor/skills/dag-task-runner/scripts" \
    "$HOME/.claude/skills/dag-task-runner/scripts"
  do
  ...
  echo "Could not find dag-task-runner/scripts. Copy the skill into this project, install it under ~/.cursor/skills or ~/.claude/skills, or set DAG_RUNNER_DIR." >&2
  return 1
}
